...
public class GraphEvalTiming
{
				...
				switch (command.toUpperCase()) {
					...
					case "U":
						if (tokens.length == 4) {
							...
							System.out.println("Update completed in " + estimatedTime + " seconds.");
					...
					case "RV":
						if (tokens.length == 2) {
							...
							long endTime = System.nanoTime();
							double estimatedTime = ((double) (endTime - startTime)) / Math.pow(10,9);
							System.out.println("Removal completed in " + estimatedTime + " seconds.");
						} else {
							System.err.println(lineNum + ": incorrect number of tokens.");
						}
						break;
					...
					case "ON":
						if (tokens.length == 3) {
							...
							long endTime = System.nanoTime();
							double estimatedTime = ((double) (endTime - startTime)) / Math.pow(10,9);
							System.out.println("Out neighbours found in " + estimatedTime + " seconds.");
					...
					case "IN":
						if (tokens.length == 3) {
							...
							long endTime = System.nanoTime();
							double estimatedTime = ((double) (endTime - startTime)) / Math.pow(10,9);
							System.out.println("In neighbours found in " + estimatedTime + " seconds.");
							StringBuffer buf = new StringBuffer();
							for (MyPair neigh : neighbours) {
								buf.append(" (" + neigh.getKey() + "," + neigh.getValue() + ")");
							}
	...
	public static void main(String[] args) {
		...
		switch(implementationType) {
			...
			case "incmat":
				...
				// For second parameter, pick 0 (low), 1 (medium) or 2 (high).
				growGraph(graph, 2);
				break;
			default:
	...
	public static void growGraph(AssociationGraph g, int density) {
		int vertAmt = 100;
		int edgeAmt = 0;
		...
		// Created with the help of: https://www.mkyong.com/java/java-generate-random-integers-in-a-range/.
		Random r = new Random();
		
		for (int i = 0; i < edgeAmt; i++) {
			int src = r.nextInt((vertAmt - 1) + 1) + 1;
			boolean valid = false;
			int tar = -1;
			while (valid == false) {
				tar = r.nextInt((vertAmt - 1) + 1) + 1;
				if (tar != src) {
					valid = true;
				}
			}
			int weight = r.nextInt((100 - 1) + 1) + 1;
			g.addEdge(Integer.toString(src), Integer.toString(tar), weight);
		}
	}
